...
def generate_population(input_tensor, temperature=0.5):
    ...
    return torch.cat([ethnicity, religion, hh_comp, hh_size], dim=-1)
...
def combined_rmse_loss(aggregated_tensor1, aggregated_tensor2, aggregated_tensor3,
                       ...
                       target_tensor1, target_tensor2, target_tensor3):
...
if training:
    ...
    for epoch in range(number_of_epochs+1):
        ...
        accuracy = (accuracy6 + accuracy7 + accuracy8) / 3
    ...
    records = decode_tensor(encoded_population, category_dicts.values())
    hh_df = pd.DataFrame(records, columns=['religion', 'ethnicity', 'hh_comp', 'hh_size'])
