...
# Using the function to initialize the input tensor
dicts = [sex_dict, age_dict, ethnic_dict, religion_dict, marital_dict, qual_dict]
category_lengths_list = [len(sex_dict), len(age_dict), len(ethnic_dict), len(religion_dict), len(marital_dict), len(qual_dict)]
input_tensor = initialize_input_tensor(total, category_lengths, dicts)
...
# print(decode_tensor(encoded_population, [sex_dict, age_dict, ethnic_dict, religion_dict, marital_dict, qual_dict]))
...
persons_df.to_csv('persons_final.csv', index=False)
